...
block_i = np.array([1.5, 1, 0])
block_f = np.array([1, -2, 0])
...
cube_pos_i = np.array([1.5, 1.0, 0.025])
cube_pos_f = np.array([1, -2, 0.025])
...
def MobileManipulatiion(cube_init, cube_final, youBot_init, T_se_init, Kp=np.zeros((4, 4)), Ki=np.zeros((4, 4))) -> None:
    ...
    T_sc_goal = np.array([[1, 0, 0,     cube_final[0]],
                          [0, 1, 0,    cube_final[1]],
                          [0, 0, 1,     cube_final[2]],
                          [0, 0, 0,                 1]])
...
def main():
    """Main function."""
    logging.basicConfig(filename='newTask_log.txt', level=logging.INFO)
    logging.info('$ python3 code/newTask.py')
    cube_init = cube_pos_i
    cube_final = cube_pos_f
    youBot_init = [0.35, -0.5, 0.3, 0, -0.149,
    ...
    WriteCSV(states, 'newTask.csv')

    logging.info('Writing error plot data.')
    WriteError(error, 'newTask_error_data.txt')
